...
function load() {
  ...
  $.indream.async = {
    ...
    //lock: 锁的编号
    //action: 解锁后执行的方法
    lock: function (lock, data, action) {

      $.indream.async.waitings[lock] = $.indream.async.waitings[lock] || [];

      $.indream.async.waitings[lock].push({
        action,
        data
      });
      ...
      if (!$.indream.async.lockStatus[lock] && action) {
        ...
        if (arguments.length > 2) {
          ...
          eval('$.indream.async.action.call(action, arguments[1])');
    ...
    releaseLock: function (lock) {
      ...
      if ($.indream.async.waitings[lock].length) {
        $.indream.async.waitings[lock][0].action($.indream.async.waitings[lock][0].data);
      } else {
    ...
    wait: function (lock, action) {
      ...
      $.indream.async.waitings[code].push({
        action,
        data
      });
    },
  ...
  $.indream.async.unlock = $.indream.async.releaseLock;
};

load();

module.exports = $.indream.async;
